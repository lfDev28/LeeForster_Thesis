...
# DSTR Sequence Stop (DSS) bit of MEASUREMENT_DSTR_STATUS->DMS
DSTR_STATUS_DSS_MASK = 0x01
# FIFO Overflow Error (FOE) bit of MEASUREMENT_DSTR_STATUS->DMS
DSTR_STATUS_FOE_MASK = 0x02
# Internal Error (IERR) bit of MEASUREMENT_DSTR_STATUS->DMS
...
class MeasConfigType(ctypes.Structure):
    ...
    _fields_ = [("m_StartPixel", ctypes.c_uint16),
                ...
                # nesting of types does NOT work!!
...
def AVS_UpdateETHDevices(spectrometers=1):
    ...
    """
    prototype = func(ctypes.c_int, ctypes.c_int, ctypes.POINTER(
        ctypes.c_int), ctypes.POINTER(BroadcastAnswerType*spectrometers))
    paramflags = (1, "listsize",), (2, "requiredsize",), (2, "ETHlist",),
    PT_AVS_UpdateETHDevices = prototype(
        ("AVS_UpdateETHDevices", lib), paramflags)
...
def AVS_GetList(spectrometers=1):
    ...
    """
    prototype = func(ctypes.c_int, ctypes.c_int, ctypes.POINTER(
        ctypes.c_int), ctypes.POINTER(AvsIdentityType*spectrometers))
...
def AVS_GetVersionInfo(handle):
    ...
    encoded in c_char
    """
    prototype = func(ctypes.c_int, ctypes.c_int, ctypes.c_char * VERSION_LEN,
                     ctypes.c_char * VERSION_LEN, ctypes.c_char * VERSION_LEN)
    paramflags = (1, "handle",), (2, "FPGAversion",), (2,
                                                       "FWversion",), (2, "DLLversion",),
...
class AVS_MeasureCallbackFunc(object):
    def __init__(self, function):
        self.prototype = ctypes.CFUNCTYPE(None, ctypes.POINTER(
...
def AVS_SetDstrStatusCallback(handle, cb):
    ...
    AVS_SetDstrStatusCallback = prototype(
        ("AVS_SetDstrStatusCallback", lib), paramflags)
...
def AVS_GetScopeData(handle):
    ...
    """
    prototype = func(ctypes.c_int, ctypes.c_int, ctypes.POINTER(
        ctypes.c_uint32), ctypes.POINTER(ctypes.c_double * 4096))
...
def AVS_GetSaturatedPixels(handle):
    ...
    """
    prototype = func(ctypes.c_int, ctypes.c_int,
                     ctypes.POINTER(ctypes.c_uint8 * 4096))
    paramflags = (1, "handle",), (2, "saturated",),
    AVS_GetSaturatedPixels = prototype(
        ("AVS_GetSaturatedPixels", lib), paramflags)
...
def AVS_SetPwmOut(handle, portId, frequency, dutycycle):
    ...
    :return: SUCCESS = 0 or FAILURE <> 0 
    """
    prototype = func(ctypes.c_int, ctypes.c_int, ctypes.c_uint8,
                     ctypes.c_uint32, ctypes.c_uint8)
    paramflags = (1, "handle",), (1, "portId",), (1,
                                                  "frequency",), (1, "dutycycle",),
...
def AVS_GetParameter(handle, size=63484):
    ...
    """
    prototype = func(ctypes.c_int, ctypes.c_int, ctypes.c_uint32, ctypes.POINTER(
        ctypes.c_uint32), ctypes.POINTER(DeviceConfigType))
    paramflags = (1, "handle",), (1, "size",), (2,
                                                "reqsize",), (2, "deviceconfig",),
...
def AVS_GetDetectorName(handle, SensorType):
    ...
    """
    prototype = func(ctypes.c_int, ctypes.c_int, ctypes.c_byte,
                     ctypes.c_char * DETECTOR_NAME_LEN)
...
def AVS_SetSensitivityMode(handle, enable):
    ...
    paramflags = (1, "handle",), (1, "enable",),
    AVS_SetSensitivityMode = prototype(
        ("AVS_SetSensitivityMode", lib), paramflags)
